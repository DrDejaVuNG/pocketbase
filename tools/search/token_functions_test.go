	...
	scenarios := []struct {
		...
		},
		{
			...
			&ResolverResult{
				...
				Identifier:   `(6371 * acos(LEAST(1, GREATEST(-1, cos(radians({:latA})) * cos(radians({:latB})) * cos(radians({:lonB}) - radians({:lonA})) + sin(radians({:latA})) * sin(radians({:latB}))))))`,
